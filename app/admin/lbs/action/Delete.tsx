...
function Delete({ lbsId, reload }: { lbsId: Number, reload: Function }) {
    ...
    const handleDelete = async (tpsId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/lbs/${lbsId}`)
        reload()
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(lbsId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
