...
function Delete({ penyulangId, reload }: { penyulangId: Number, reload: Function }) {
    ...
    const handleDelete = async (karyawanId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/penyulang/${penyulangId}`)
        setTimeout(function () {
            setIsLoading(false)
            Swal.fire({
                position: 'top-end',
        ...
        <>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(penyulangId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </>
    )
