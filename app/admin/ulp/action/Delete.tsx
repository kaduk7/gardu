...
function Delete({ ulpId, reload }: { ulpId: Number, reload: Function }) {
    ...
    const handleDelete = async (karyawanId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/ulp/${ulpId}`)
        setTimeout(function () {
            setIsLoading(false)
            Swal.fire({
                position: 'top-end',
        ...
        <>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(ulpId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </>
    )
